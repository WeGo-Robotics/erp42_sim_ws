...
class ERP42_ctrl_cmd:
    ...
    def main(self):
        ...
        speed = 15  # 0 ~ 23.8 -> 0 ~ 1
        # Adjust speed value to within the range of 0 ~ 23.8
        speed = max(0, min(speed, 23.8))
        # Steering angle (left: positive, right: negative)
        steering_deg = 10
        # Adjust within the range of steering limit values
        steering_deg = max(-self.steering_bound, min(steering_deg, self.steering_bound))
        # Change angle to radians
        steering_rad = steering_deg * pi / 180
        # Since the sign of the ERP42 control message and the steering angle are opposite, reverse the sign.
        ...
        if self.second_time - self.start_time >= 3:
            ...
            # Publish the message object to the "/ctrl_cmd" topic.
